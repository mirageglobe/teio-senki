...
| persistence model: snapshot-per-turn             | serialise full ledger to `save.json` at end of each cycle C                         | game flow is strictly linear (A→B→C→A); current state is always the last snapshot — no replay needed. event sourcing was considered: richer audit trail and self-verifying saves, but the fold (events→state) grows with every new event type and adds complexity with no player-visible benefit. snapshot-per-turn is correct, bounded, and fast to load. `Logs []LogEntry` remains append-only for audit trail and victory scoring — not the source of truth |
...
- `[engine]` **event sourcing ledger** — replace `LogEntry{Type, Description string}` with a typed payload struct per event kind (e.g. `SeasonShift`, `BuildExecuted`, `YieldSettled`). current state becomes a pure fold over the event log. enables: turn replay, save/load from event log only, full audit trail, deterministic simulation. prerequisite: freeze the event taxonomy before M3 city economics ships, or migration cost grows fast [medium]
