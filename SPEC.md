...
| **rendering** | Ebitengine | 2D game loop, input handling, image drawing |
...
| **testing** | `go test ./...` | standard Go test runner; no engine dependency for core logic |
| **build** | GNU Make + Go toolchain | `make run`, `make test`, `make build`, `make export-*` |
...
cmd/
  teio/
    main.go             # entry point: load data, init ledger, run Ebitengine loop
internal/
  ...
  ui/                   # Ebitengine rendering — imports engine, never vice versa
    map/                # strategic map screen (permanent primary surface)
...
- **headless engine**: all simulation logic (`internal/core`, `internal/engine`) has zero Ebitengine imports. testable with `go test ./internal/...` — no game loop required.
...
- **ui minimalism**: use placeholder Ebitengine rectangle-and-text drawing to verify engine logic before committing to pixel art assets.
...
| :--- | :--- | :--- | :--- |
| **1: genesis** | foundation, data pipeline, and turn engine | 0, 1 | rewrite (Go + Ebitengine) |
| **2: cartography** | strategic map and visual world | 2 | rewrite (Go + Ebitengine) |
| **3: governance** | city development and economics | 3 | rewrite (Go + Ebitengine) |
...
| :--- | :--- | :--- |
| 0 | foundation | rewrite |
| 1 | data + turn engine (focus: AD 189) | rewrite |
| 2 | strategic map (visualizing china) | rewrite |
| 3 | cities (development & economics) | rewrite |
...
| decision | choice | why |
| :--- | :--- | :--- |
| Ebitengine over Godot / Defold | Go + Ebitengine | long-term investment; Go is primary language across the stack. typed structs beat GDScript dicts for complex simulation. `go test` is simpler than `godot --headless`. Defold ruled out: Lua has no type safety. Godot M0–M3 prototype informs the rewrite |
