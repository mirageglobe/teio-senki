...
**city `[seasonal]`:** `develop <city_id> <pillar>` — grow AG, COM, or DEF. pillar growth is applied once per season; issuing multiple develop commands in the same season queues them but only the seasonal tick applies the delta.
...
**army:** `recruit <city_id> <amount> [seasonal]` — raise troops from city population; only available on seasonal turns. `march <army_id> <x> <y>` — move army every month; range = `general.strategy ÷ 20` tiles. `encamp <army_id>` — fortify position.

**diplomacy:** `tribute <faction>` / `alliance <faction>` / `threaten <faction>` — modifies faction relation score (−100 to +100). costs gold, no CP.

> commands marked `[seasonal]` are only available on the first turn of each season (months 1, 4, 7, 10). all other commands are available every month.
